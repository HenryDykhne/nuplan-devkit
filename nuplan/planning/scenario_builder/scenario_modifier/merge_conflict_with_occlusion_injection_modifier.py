...
class MergeConflictWithOcclusionInjectionModifier(ConflictInjectionAndOcclusionInjectionModifier):
    ...
    def modify_scenario(self, runner: SimulationRunner) -> List[SimulationRunner]:
        ...
        # selecting for only merging connectors
        conflicting_merging_connectors = [connector for connector in conflicting_connectors if (connector.outgoing_edges[0].id == crossing_lane_connector.outgoing_edges[0].id)]
        valid_conflicting_connectors = [connector for connector in conflicting_merging_connectors if connector.id not in traffic_light_status[TrafficLightStatusType.RED]]
        ...
        for conflict_connector in valid_conflicting_connectors:
            ...
            for conflict_vehicle_speed in conflict_vehicle_speeds:
                ...
                *_, last = goal_segment.baseline_path.linestring.coords #we use the last point in a linestring far away to start setting our goal
                ...
                conflict_vehicle_token = "merge_conflict_inserted"
                ...
                for point in candidate_occluding_spawn_points:
                    ...
                    candidate, goal = self.generate_occlusion_candidate_merge(point, runner, scenario.map_api, traffic_light_status, [conflict_agent_to_insert]) #occlusions candidates for merge conflicts can be done similarily to those for cross conflicts
...
class MergeConflictWithOcclusionInjectionModification(AbstractModification):
